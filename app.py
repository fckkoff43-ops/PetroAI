...
st.set_page_config(page_title='PetroAI Analyzer', page_icon='🛢️', layout='wide')
PETRO='#c9922e'; STEEL='#4f7f9e'; BLACK='#0a0e13'; PANEL='#121a22'; MUTED='#8492a1'
ALERT='#b5462f'; GOOD='#3f8f74'; NEUTRAL='#5b6672'
...
st.markdown(f'''<style>
.stApp{{background:radial-gradient(circle at 85% 0%,rgba(79,127,158,.10),transparent 28%),{BLACK};color:#eef1f4}}
[data-testid="stHeader"]{{background:transparent}}[data-testid="stToolbar"]{{visibility:hidden}}footer{{visibility:hidden}}
.block-container{{max-width:1500px;padding-top:1rem}}
section[data-testid="stSidebar"]{{background:linear-gradient(180deg,#080c11,#0d131a 70%,#11161c);border-right:1px solid rgba(79,127,158,.25)}}
.brand{{padding:8px 6px 18px;border-bottom:1px solid #1c2530;margin-bottom:15px}}.brand-title{{font-size:30px;font-weight:900}}.brand-title span{{color:{PETRO}}}.brand-sub{{font-size:11px;color:{MUTED}}}
.hero{{min-height:210px;border:1px solid rgba(79,127,158,.4);border-radius:10px;overflow:hidden;background-image:linear-gradient(90deg,rgba(10,14,19,.97),rgba(10,14,19,.82) 38%,rgba(10,14,19,.35) 78%,rgba(10,14,19,.75)),url('{uri}');background-size:cover;background-position:center;margin-bottom:14px}}
.hero-inner{{padding:26px 28px}}.hero h1{{font-size:42px;margin:2px 0}}.hero h1 span{{color:{PETRO}}}.hero p{{color:#aeb9c4;max-width:650px}}.tag{{font-weight:700;font-style:italic;color:{STEEL}}}
.asset{{display:inline-block;background:rgba(18,26,34,.85);border:1px solid #263241;border-radius:6px;padding:7px 11px;margin:8px 5px 0 0;font-size:12px}}
.card,.panel{{background:linear-gradient(145deg,#131b23,#0d131a);border:1px solid rgba(255,255,255,.07);border-radius:10px;padding:16px;box-shadow:0 7px 20px rgba(0,0,0,.25)}}.card{{min-height:100px;border-top:2px solid {STEEL}}}.orange{{border-top-color:{PETRO}}}.label{{font-size:11px;color:#9aa7b4;letter-spacing:.02em}}.value{{font-size:27px;font-weight:800;margin-top:5px}}.note{{font-size:11px;color:{MUTED}}}
.finding{{border:1px solid rgba(79,127,158,.35);background:rgba(79,127,158,.07);border-radius:8px;padding:12px;margin:7px 0}}.finding b{{color:{PETRO}}}
.footer{{border-top:1px solid #1c2530;margin-top:28px;padding-top:12px;color:{MUTED};font-size:11px}}.section-title{{font-size:18px;font-weight:800;margin:22px 0 10px;padding-left:10px;border-left:4px solid {STEEL}}}.risk-score{{display:flex;align-items:baseline;gap:6px;margin:8px 0}}.risk-number{{font-size:48px;font-weight:900;color:{PETRO}}}.risk-badge{{display:inline-block;background:linear-gradient(90deg,{STEEL},{PETRO});padding:5px 12px;border-radius:5px;font-size:11px;font-weight:800;letter-spacing:.03em;margin-bottom:8px}}.mini-risk{{font-size:11px;border-bottom:1px solid #1c2530;padding:7px 0;color:#c7d0d9}}.action{{font-size:11px;color:#b9c3cd;padding:5px 0}}.trend-card{{background:#121a22;border:1px solid rgba(255,255,255,.07);border-radius:8px;padding:13px;border-top:2px solid {NEUTRAL}}}.trend-bad{{border-top-color:{ALERT}}}.trend-good{{border-top-color:{GOOD}}}.trend-neutral{{border-top-color:{NEUTRAL}}}.trend-name{{font-size:16px;font-weight:800;margin:5px 0}}.report-panel{{display:flex;justify-content:space-between;align-items:center;margin-top:18px;border-color:rgba(79,127,158,.3)}}
.stDownloadButton>button{{background:linear-gradient(90deg,{STEEL},{PETRO});color:#fff;border:0;font-weight:700}}
[data-testid="stFileUploaderDropzone"]{{background:linear-gradient(145deg,#131b23,#0d131a);border:1.5px dashed rgba(79,127,158,.5);border-radius:10px}}
[data-testid="stFileUploaderDropzoneInstructions"] span,[data-testid="stFileUploaderDropzoneInstructions"] small{{color:{MUTED}}}
[data-testid="stFileUploaderFile"]{{background:{PANEL};border:1px solid rgba(255,255,255,.07);border-radius:6px;color:#eef1f4}}
[data-testid="stExpander"]{{background:linear-gradient(145deg,#131b23,#0d131a);border:1px solid rgba(255,255,255,.07);border-radius:10px}}
[data-testid="stExpander"] summary{{color:#eef1f4}}
[data-testid="stAlert"]{{background:linear-gradient(145deg,#131b23,#0d131a);border:1px solid rgba(255,255,255,.07);border-radius:8px}}
[data-testid="stDataFrame"],[data-testid="stDataFrameResizable"]{{border:1px solid rgba(255,255,255,.07);border-radius:8px;overflow:hidden}}
.stButton>button{{background:{PANEL};color:#eef1f4;border:1px solid rgba(79,127,158,.35);font-weight:600}}
.stButton>button:hover{{border-color:{STEEL};color:#fff}}
...
st.markdown('<div class="hero"><div class="hero-inner"><div>🛢️ Welcome to</div><h1>Petro<span>AI</span> Analyzer</h1><p>Analyze well production performance, identify water-related risks, and get petroleum engineering recommendations from production data.</p><div class="tag">Smarter Analysis. Better Decisions. Higher Production.</div></div></div>',unsafe_allow_html=True)
...
NAVY=colors.HexColor('#1c2733');STEELc=colors.HexColor('#4f7f9e');PETROc=colors.HexColor('#c9922e');MUTEDc=colors.HexColor('#6b7784');ALERTc=colors.HexColor('#b5462f');GOODc=colors.HexColor('#3f8f74');LINEc=colors.HexColor('#d9dee3');ROWc=colors.HexColor('#f4f6f8')
_status_color={'GOOD':GOODc,'MONITOR':STEELc,'ATTENTION':PETROc,'CRITICAL':ALERTc}
_risk_color={'LOW RISK':GOODc,'MEDIUM RISK':STEELc,'HIGH RISK':PETROc,'CRITICAL RISK':ALERTc}

def _pdf_styles():
    s=getSampleStyleSheet()
    return {
        'cover_title':ParagraphStyle('cover_title',fontName='Helvetica-Bold',fontSize=27,textColor=NAVY,leading=32),
        'cover_sub':ParagraphStyle('cover_sub',fontName='Helvetica',fontSize=12.5,textColor=STEELc,spaceAfter=2,leading=16),
        'cover_meta':ParagraphStyle('cover_meta',fontName='Helvetica',fontSize=9.5,textColor=MUTEDc,leading=15),
        'h':ParagraphStyle('h',fontName='Helvetica-Bold',fontSize=13.5,textColor=NAVY,spaceBefore=16,spaceAfter=8),
        'body':ParagraphStyle('body',fontName='Helvetica',fontSize=10,textColor=colors.HexColor('#26313c'),leading=14.5,spaceAfter=4),
        'bullet':ParagraphStyle('bullet',fontName='Helvetica',fontSize=10,textColor=colors.HexColor('#26313c'),leading=14.5,leftIndent=10,spaceAfter=3),
        'label':ParagraphStyle('label',fontName='Helvetica-Bold',fontSize=9,textColor=colors.white),
        'kpi_val':ParagraphStyle('kpi_val',fontName='Helvetica-Bold',fontSize=17,textColor=NAVY),
        'kpi_lab':ParagraphStyle('kpi_lab',fontName='Helvetica',fontSize=8,textColor=MUTEDc),
    }

def _pdf_header_footer(canvas,doc):
    canvas.saveState()
    canvas.setStrokeColor(LINEc);canvas.setLineWidth(.6);canvas.line(1.5*cm,A4[1]-1.15*cm,A4[0]-1.5*cm,A4[1]-1.15*cm)
    canvas.setFont('Helvetica-Bold',8);canvas.setFillColor(NAVY);canvas.drawString(1.5*cm,A4[1]-1*cm,'PetroAI Analyzer')
    canvas.setFont('Helvetica',8);canvas.setFillColor(MUTEDc);canvas.drawRightString(A4[0]-1.5*cm,A4[1]-1*cm,'Petroleum Production Engineering Report')
    canvas.setStrokeColor(LINEc);canvas.line(1.5*cm,1.25*cm,A4[0]-1.5*cm,1.25*cm)
    canvas.setFont('Helvetica',8);canvas.setFillColor(MUTEDc)
    canvas.drawString(1.5*cm,.9*cm,'Generated by PetroAI • Rule-Based Expert System')
    canvas.drawRightString(A4[0]-1.5*cm,.9*cm,f'Page {doc.page}')
    canvas.restoreState()

def _kpi_cell(val,lab,st_):
    return Table([[Paragraph(val,st_['kpi_val'])],[Paragraph(lab,st_['kpi_lab'])]],colWidths=[4.05*cm],
                 style=TableStyle([('BACKGROUND',(0,0),(-1,-1),colors.HexColor('#f7f8fa')),('BOX',(0,0),(-1,-1),.6,LINEc),
                                    ('TOPPADDING',(0,0),(-1,-1),10),('BOTTOMPADDING',(0,0),(-1,-1),10),
                                    ('LEFTPADDING',(0,0),(-1,-1),12),('ALIGN',(0,0),(-1,-1),'LEFT')]))

def make_pdf_report():
    st_=_pdf_styles()
    b=BytesIO()
    doc=SimpleDocTemplate(b,pagesize=A4,rightMargin=1.5*cm,leftMargin=1.5*cm,topMargin=1.7*cm,bottomMargin=1.7*cm)
    period=f"{first['Date'].strftime('%d %b %Y')} – {last['Date'].strftime('%d %b %Y')}"

    # ---------- COVER ----------
    story=[Spacer(1,1.2*cm)]
    if ASSET.exists(): story+=[Image(str(ASSET),width=5*cm,height=5*cm*0.28) if False else Spacer(1,0)]
    story+=[Paragraph('PetroAI Analyzer',st_['cover_title']),Paragraph('Petroleum Production Engineering Report',st_['cover_sub']),
            Spacer(1,.4*cm),HRFlowable(width='100%',thickness=1.4,color=STEELc,spaceAfter=14)]
    meta=[['Analysis period',period],['Data points',f"{len(df)} records"],['Source file',getattr(uploaded,'name','—')],
          ['Generated on',pd.Timestamp.now().strftime('%d %B %Y, %H:%M')]]
    mt=Table(meta,colWidths=[4.2*cm,10.8*cm]);mt.setStyle(TableStyle([('FONTNAME',(0,0),(0,-1),'Helvetica-Bold'),('FONTNAME',(1,0),(1,-1),'Helvetica'),
        ('FONTSIZE',(0,0),(-1,-1),9.5),('TEXTCOLOR',(0,0),(0,-1),MUTEDc),('TEXTCOLOR',(1,0),(1,-1),NAVY),
        ('BOTTOMPADDING',(0,0),(-1,-1),5),('TOPPADDING',(0,0),(-1,-1),0)]))
    story+=[mt,Spacer(1,.9*cm)]
    kpis=[_kpi_cell(f"{df['Oil Rate'].mean():,.1f}",'AVG OIL RATE',st_),_kpi_cell(f"{df['Water Cut (%)'].mean():.1f}%",'AVG WATER CUT',st_),
           _kpi_cell(f"{df.WOR.mean():.2f}",'AVG WOR',st_),_kpi_cell(f'{score}/100',f'HEALTH — {status}',st_)]
    kt=Table([kpis],colWidths=[4.05*cm]*4,style=TableStyle([('LEFTPADDING',(0,0),(-1,-1),0),('RIGHTPADDING',(0,0),(-1,-1),6)]))
    story+=[kt,Spacer(1,.6*cm),
            Paragraph('This report summarizes production performance, trend diagnostics, risk assessment, and '
                      'engineering recommendations generated from the uploaded production dataset.',st_['body']),
            PageBreak()]

    # ---------- 1. PRODUCTION SUMMARY ----------
    story.append(Paragraph('1 &nbsp;·&nbsp; Production Summary',st_['h']))
    story.append(HRFlowable(width='100%',thickness=.6,color=LINEc,spaceAfter=8))
    data=[['Parameter','Average'],['Oil Rate',f"{df['Oil Rate'].mean():.2f}"],['Water Rate',f"{df['Water Rate'].mean():.2f}"],
          ['Water Cut',f"{df['Water Cut (%)'].mean():.2f}%"],['WOR',f"{df.WOR.mean():.2f}"]]
    t=Table(data,colWidths=[8*cm,8*cm])
    t.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),NAVY),('TEXTCOLOR',(0,0),(-1,0),colors.white),('FONTNAME',(0,0),(-1,0),'Helvetica-Bold'),
        ('FONTNAME',(0,1),(-1,-1),'Helvetica'),('FONTSIZE',(0,0),(-1,-1),9.5),('ROWBACKGROUNDS',(0,1),(-1,-1),[colors.white,ROWc]),
        ('GRID',(0,0),(-1,-1),.5,LINEc),('TOPPADDING',(0,0),(-1,-1),7),('BOTTOMPADDING',(0,0),(-1,-1),7),('LEFTPADDING',(0,0),(-1,-1),10)]))
    story.append(t)

    # ---------- 2. PERFORMANCE SCORE ----------
    story.append(Paragraph('2 &nbsp;·&nbsp; Performance Score',st_['h']))
    story.append(HRFlowable(width='100%',thickness=.6,color=LINEc,spaceAfter=8))
    badge=Table([[Paragraph(f'<b>{score}/100</b>',ParagraphStyle('b',fontName='Helvetica-Bold',fontSize=13,textColor=colors.white)),
                  Paragraph(status,ParagraphStyle('s',fontName='Helvetica-Bold',fontSize=10,textColor=colors.white))]],
                colWidths=[3*cm,4*cm],style=TableStyle([('BACKGROUND',(0,0),(-1,-1),_status_color.get(status,STEELc)),
                ('VALIGN',(0,0),(-1,-1),'MIDDLE'),('TOPPADDING',(0,0),(-1,-1),8),('BOTTOMPADDING',(0,0),(-1,-1),8),
                ('LEFTPADDING',(0,0),(0,-1),12),('LEFTPADDING',(1,0),(1,-1),0)]))
    story+=[badge,Spacer(1,4)]

    # ---------- 3. TREND ANALYSIS ----------
    story.append(Paragraph('3 &nbsp;·&nbsp; Trend Analysis',st_['h']))
    story.append(HRFlowable(width='100%',thickness=.6,color=LINEc,spaceAfter=8))
    trend_rows=[['Parameter','Trend','Normalized Slope']]+[[n,tr,f'{s:.3f}%'] for n,tr,s in
        [('Oil Rate',oil_trend,oil_slope),('Water Rate',water_trend,water_slope),('Water Cut',wc_trend,wc_slope),('WOR',wor_trend,wor_slope)]]
    tt=Table(trend_rows,colWidths=[5.3*cm,5.3*cm,5.4*cm])
    ...
    # ---------- 4. RISK ASSESSMENT ----------
    story.append(Paragraph('4 &nbsp;·&nbsp; Risk Assessment',st_['h']))
    story.append(HRFlowable(width='100%',thickness=.6,color=LINEc,spaceAfter=8))
    rbadge=Table([[Paragraph(f'<b>{risk}/11</b>',ParagraphStyle('b2',fontName='Helvetica-Bold',fontSize=13,textColor=colors.white)),
                   Paragraph(risk_level,ParagraphStyle('s2',fontName='Helvetica-Bold',fontSize=10,textColor=colors.white))]],
                 colWidths=[3*cm,4.5*cm],style=TableStyle([('BACKGROUND',(0,0),(-1,-1),_risk_color.get(risk_level,STEELc)),
                 ('VALIGN',(0,0),(-1,-1),'MIDDLE'),('TOPPADDING',(0,0),(-1,-1),8),('BOTTOMPADDING',(0,0),(-1,-1),8),
                 ('LEFTPADDING',(0,0),(0,-1),12),('LEFTPADDING',(1,0),(1,-1),0)]))
    story+=[rbadge,Spacer(1,8)]
    if reasons:
        for r in reasons: story.append(Paragraph('▪ '+r,st_['bullet']))
    else:
        story.append(Paragraph('No major risk factor detected.',st_['body']))

    # ---------- 5. ENGINEERING RECOMMENDATION ----------
    story.append(Paragraph('5 &nbsp;·&nbsp; Engineering Recommendation',st_['h']))
    story.append(HRFlowable(width='100%',thickness=.6,color=LINEc,spaceAfter=8))
    story.append(Paragraph(f'<b>{rec_title}</b>',st_['body']))
    story.append(Paragraph(rec_text,st_['body']))
    story.append(Spacer(1,4))
    for x in actions: story.append(Paragraph('▪ '+x,st_['bullet']))

    # ---------- 6. PRODUCTION CHARTS ----------
    story.append(PageBreak())
    story.append(Paragraph('6 &nbsp;·&nbsp; Production Charts',st_['h']))
    story.append(HRFlowable(width='100%',thickness=.6,color=LINEc,spaceAfter=10))
    chart_color={'Oil Rate':'#4f7f9e','Water Rate':'#c9922e','Water Cut (%)':'#4f7f9e','WOR':'#c9922e'}
    plt.rcParams.update({'font.size':9,'axes.edgecolor':'#c7cdd3','axes.labelcolor':'#3a4552','text.color':'#1c2733',
                          'xtick.color':'#6b7784','ytick.color':'#6b7784'})
    for col in ['Oil Rate','Water Rate','Water Cut (%)','WOR']:
        fig,ax=plt.subplots(figsize=(7.2,3.2))
        ax.plot(df.Date,df[col],marker='o',markersize=3,linewidth=1.6,color=chart_color[col])
        ax.set_title(col,fontsize=11,fontweight='bold',color='#1c2733',loc='left')
        ax.set_xlabel('Date');ax.set_ylabel(col)
        ax.grid(True,alpha=.25,color='#c7cdd3')
        for spine in ['top','right']: ax.spines[spine].set_visible(False)
        fig.autofmt_xdate()
        ib=BytesIO();fig.savefig(ib,format='png',dpi=160,bbox_inches='tight',facecolor='white');plt.close(fig);ib.seek(0)
        story+=[Image(ib,width=17*cm,height=7.6*cm),Spacer(1,.35*cm)]
...
if page=='Dashboard':
    ...
    c1,c2=st.columns(2)
    with c1:
        fig=go.Figure();fig.add_trace(go.Scatter(x=df.Date,y=df['Oil Rate'],mode='lines+markers',name='Oil Rate',line=dict(color=PETRO,width=3)));fig.add_trace(go.Scatter(x=df.Date,y=df['Water Rate'],mode='lines+markers',name='Water Rate',line=dict(color=STEEL,width=3)));fig.update_layout(template='plotly_dark',height=360,title='Production Trend',paper_bgcolor='rgba(0,0,0,0)',plot_bgcolor='rgba(0,0,0,0)');st.plotly_chart(fig,use_container_width=True)
    with c2:
        fig=go.Figure();fig.add_trace(go.Scatter(x=df.Date,y=df['Water Cut (%)'],mode='lines+markers',name='Water Cut',line=dict(color=PETRO,width=3)));fig.add_trace(go.Scatter(x=df.Date,y=df.WOR,mode='lines+markers',name='WOR',yaxis='y2',line=dict(color=STEEL,width=3)));fig.update_layout(template='plotly_dark',height=360,title='Water Cut & WOR',yaxis=dict(title='Water Cut (%)'),yaxis2=dict(title='WOR',overlaying='y',side='right'),paper_bgcolor='rgba(0,0,0,0)',plot_bgcolor='rgba(0,0,0,0)');st.plotly_chart(fig,use_container_width=True)
    ...
    for col in ['Oil Rate','Water Rate']:
        fig=go.Figure(go.Scatter(x=df.Date,y=df[col],mode='lines+markers',line=dict(color=PETRO if col=='Oil Rate' else STEEL,width=3)));fig.update_layout(template='plotly_dark',height=350,title=col+' vs Time',paper_bgcolor='rgba(0,0,0,0)',plot_bgcolor='rgba(0,0,0,0)');st.plotly_chart(fig,use_container_width=True)

elif page=='Water Cut & WOR':
    c1,c2=st.columns(2)
    for c,col,title,color in [(c1,'Water Cut (%)','Water Cut vs Time',PETRO),(c2,'WOR','WOR vs Time',STEEL)]:
        with c:
